...
cert_label() {(
  # Generate an informative label for a cert/key/encryption file.
  set -eu -o pipefail
  md5() {
    # cat
  ...
  for filepath in "$@"; do
    ...
    echo Unknown file: $fileinfo 1>&2
    return 1
  done
)}

cert_extract() {(
  # Extract certificates and keys from archive formats
  set -eu -o pipefail
  md5() {
    # cat
    openssl md5 | cut -d ' ' -f 2-
...
cert_label_hoist() {(
  # Rename certificates to their labels, and link the old location to the new one.
  set -eu -o pipefail
  for filepath in "$@"; do
...
cert_sign() {(
  # Attempt to sign certificates with their keys, if available
  set -eu -o pipefail
  for filepath in "$@"; do
    splitpath "$filepath"
...
cert_nickname() {(
  ...
  if [[ ! -f "$hash".key ]]; then
    echo "Missing required file:" "$hash".key
    return
  fi

  mv -v "$nickname.pem" "$nickname.pem.bak" || :;
  ln -s "$hash"*[0-9].pem "$nickname.pem"

  mv -v "$nickname.key" "$nickname.key.bak" || :;
  ln -s "$hash"*.key "$nickname.key"

  if [[ -f "$hash"*"SIGNED.pem" ]]; then
    mv -v "${nickname}_signed.pem" "$nickname_signed.pem.bak" || :;
    ln -s "$hash"*"SIGNED.pem" "$nickname_signed.pem"
  fi
