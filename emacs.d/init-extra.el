...
(add-to-list 'load-path "~/.emacs.d/vend-lisp/")
...
(defun my-package-update ()
  ...
  (let (;; Workarounds, enable as needed
    ...
    (setq package-selected-packages
      '(
         ace-window annalist ansi avy
         bind-key
         cl-generic company cfrs
         counsel
         dash dired-du dired-quick-sort
         disk-usage
         elisp-refs epl evil evil-collection edit-indirect
         f fiplr flycheck flycheck-eglot
         gnu-elpa-keyring-update
         git-modes goto-chg grizzl golden-ratio
         hyperbole hydra
         ivy
...
(autoload 'disk-usage "disk-usage" "disk-usage major mode" t)
;; (use-package window-purpose)

;; Keyquiz

(autoload 'my-key-quiz "my-keyquiz" "Emacs key quiz" t)

;; efar directory manager
(use-package efar
  :commands (efar)
  :config
...
(use-package flycheck
  ...
  (flycheck-define-checker python-pycodestyle
    ...
    :standard-input t
    :error-patterns
    ((error line-start (or "stdin" (file-name)) ":" line ":" column ":" (message) line-end))
...
(add-hook 'markdown-mode-hook (lambda ()
  ...
  (defun my-open-image-function (image-link)
    (find-file-other-window image-link))
  (setq markdown-open-image-command 'my-open-image-function)
